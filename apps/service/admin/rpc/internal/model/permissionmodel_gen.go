...
type (
	permissionModel interface {
		...
		FindOne(ctx context.Context, id uint64) (*Permission, error)
...
func (m *defaultPermissionModel) Delete(ctx context.Context, id uint64) error {
	query := fmt.Sprintf("delete from %s where `id` = ?", m.table)
	_, err := m.conn.ExecCtx(ctx, query, id)
	return err
}

func (m *defaultPermissionModel) FindOne(ctx context.Context, id uint64) (*Permission, error) {
...
func (m *defaultPermissionModel) Insert(ctx context.Context, data *Permission) (sql.Result, error) {
	...
	ret, err := m.conn.ExecCtx(ctx, query, data.Name, data.Code, data.Module)
	return ret, err
}
...
func (m *defaultPermissionModel) Update(ctx context.Context, newData *Permission) error {
	...
	_, err := m.conn.ExecCtx(ctx, query, newData.Name, newData.Code, newData.Module, newData.Id)
	return err
}
