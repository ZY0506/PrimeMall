...
CREATE TABLE IF NOT EXISTS `after_sale` (
    ...
    `status`                   TINYINT         NOT NULL DEFAULT 10 COMMENT '状态：10待审核 20待退货 30退款中 40已完成 50已拒绝 60已取消',
    ...
    `approved_amount`          BIGINT          NOT NULL DEFAULT 0 COMMENT '审核通过金额（分）',
    `real_refund_amount`       BIGINT          NOT NULL DEFAULT 0 COMMENT '实际退款金额（分）',
    ...
    `return_tracking_no`       VARCHAR(100)             DEFAULT '' COMMENT '退货物流单号',
    `return_tracking_company`  VARCHAR(50)              DEFAULT '' COMMENT '退货物流公司',
    `return_received_time`     DATETIME                 DEFAULT NULL COMMENT '商家收货时间',
    `refund_sn`                VARCHAR(64)     NOT NULL DEFAULT '' COMMENT '商户退款单号（唯一）',
    `refund_time`              DATETIME                 DEFAULT NULL COMMENT '退款时间',
    `third_refund_sn`          VARCHAR(64)              DEFAULT '' COMMENT '第三方退款单号',
    `expire_time`              DATETIME                 DEFAULT NULL COMMENT '超时时间（用于定时关单）',
    `cancel_time`              DATETIME                 DEFAULT NULL COMMENT '取消时间',
